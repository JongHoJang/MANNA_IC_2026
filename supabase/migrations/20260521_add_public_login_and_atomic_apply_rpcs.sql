create or replace function public.find_participant_by_name_phone(p_name text, p_phone text)
returns table (
  id text,
  name text,
  phone text,
...
as $$
  select
    ...
    participant.ticket_info,
    participant.day1,
    participant.day2,
    participant.day3
  from public.participants as participant
  where lower(regexp_replace(participant.name, '\s+', '', 'g')) = lower(regexp_replace(coalesce(p_name, ''), '\s+', '', 'g'))
    and regexp_replace(participant.phone, '\D', '', 'g') = regexp_replace(coalesce(p_phone, ''), '\D', '', 'g')
  limit 1;
$$;

grant execute on function public.find_participant_by_name_phone(text, text) to anon, authenticated, service_role;
